  ...
  sock.ev.process(async (events) => {
    if (events['connection.update']) {
      ...
      if (connection === 'open') {
		...
		const numero = '573232310777@s.whatsapp.net' // Asegúrate de usar el formato correcto
  		const mensaje = { text: '¡Hola! Soy un bot de prueba 🤖' }

 		 await sendMessageWTyping(mensaje, numero)
